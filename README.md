...
![QRU 홈 화면](docs/images/home.jpg)

> 아래 화면들은 모두 예시로 만든 가짜 명함입니다. 실제 사용자의 정보가 아닙니다.

## 주요 기능 ✨

### 1. 디지털 명함 생성 🖋️

![명함 생성 모달](docs/images/create-modal.jpg)
...
- **회원가입도, 비밀번호도 없습니다.** 로그인하지 않고 만들어도 그 브라우저에 신원이 조용히 발급되어, 만든 명함을 계속 관리할 수 있습니다. 나중에 Google 로그인을 하면 **같은 명함이 그대로 계정으로 옮겨집니다.**

#### 사진 맞추기

![사진 편집](docs/images/photo-editor.jpg)
...
![내 명함](docs/images/mypage.jpg)

- **만든 명함을 모아 봅니다.** 이름, 일련번호, 만든 날짜, 셔플 노출 여부가 한눈에 들어옵니다. 카드를 누르면 그 명함으로 갑니다.
- **로그인 여부와 관계없이** 쓸 수 있습니다. 로그인하지 않았다면 명함이 지금 쓰는 브라우저에 묶여 있고, 로그인하면 계정으로 옮겨져 다른 기기에서도 보입니다.
...
- 명함은 **검색 엔진에 실리지 않습니다.** 명함 · 명함 찾기 · 마이 페이지는 `X-Robots-Tag: noindex` 헤더와 `<meta name="robots">` 를 함께 내보내, 주소를 아는 사람만 볼 수 있게 합니다. 홈은 그대로 검색됩니다.
- `robots.txt` 에서 막지는 **않습니다.** 막으면 크롤러가 페이지를 열어보지 못해 `noindex` 를 읽을 수 없고, 주소만 검색 결과에 남기 때문입니다.

### 6. 그 외

다크 / 라이트 테마와, 모바일(~768px) · 태블릿(~1024px) · 데스크톱(1025px~) 레이아웃을 지원합니다.

<table>
<tr>
<td width="68%"><img src="docs/images/card-dark.jpg" alt="다크 테마 명함"></td>
<td width="32%"><img src="docs/images/mobile.jpg" alt="모바일 명함 찾기"></td>
</tr>
</table>
...
로그인은 Firebase Authentication 의 **Google 계정 로그인**을 씁니다. 로그인하지 않은 사람에게는 명함을 만드는 순간 **익명 인증**으로 uid 를 발급합니다. 소유권을 판단하는 기준이 회원과 똑같아져, 보안 규칙을 따로 두지 않고도 비회원이 자기 명함을 고치고 지울 수 있습니다. 나중에 Google 로그인을 하면 같은 uid 에 계정을 **연결**하므로 만들어 둔 명함이 그대로 따라옵니다.
...
- **Firebase Authentication**: Google 계정 로그인, 익명 로그인
- **Firebase Hosting**: 배포
...
![명함 한 장이 저장되는 모습](docs/images/data-model.png)

명함 한 장은 **공개 문서 + 하위 문서 두 개**로 나뉘고, 일련번호를 위한 **길잡이 문서**가 하나 더 붙습니다.
...
│
└── photo/data               # 사진
    └── dataUrl              # 줄여서 담은 JPEG / WebP 데이터 URL
```
...
**`guestCards` 는 무엇인가** — 익명 인증을 쓰기 전, 비밀번호로 소유권을 가리던 시절의 명함이 들어 있는 컬렉션입니다. 지금은 아무도 여기에 쓰지 않고, 규칙에서도 수정과 삭제를 막아 두었습니다. 읽기와 셔플에서만 함께 훑습니다. 정리 방안은 [#32](https://github.com/hyeoniverse/QRU/issues/32) 에서 다룹니다.
...
├── docs/                     # 컨벤션 및 트러블슈팅 문서
│   └── images/               # README 용 화면 이미지
├── scripts/
│   └── pr-screenshots.mjs    # PR 본문에 스크린샷을 붙이는 스크립트
├── src/
│   ├── api/                  # react-query 클라이언트
│   ├── components/
│   │   ├── card/             # 명함 보기 · QR 공유 · 수정/삭제
│   │   ├── common/           # 버튼, 입력, 모달, 토스트 등
│   │   ├── form/             # 명함 생성 폼, 목차, 사진 편집
│   │   ├── header/           # 내비게이션, 검색, 테마 전환
│   │   ├── home/             # 홈 화면과 생성 모달
│   │   └── layout/           # Header / Footer / Layout
│   ├── context/              # 테마 컨텍스트
│   ├── data/                 # 폼 항목 정의, 내비게이션 정의
│   ├── hooks/                # useCardForm, useNoIndex 등 커스텀 훅
│   ├── pages/                # 라우트 페이지
│   ├── services/             # firebase 초기화, 명함 읽기/쓰기
│   ├── store/                # Redux 스토어와 슬라이스
│   ├── styles/               # 전역 스타일과 테마
│   ├── types/                # 공용 타입
│   └── utils/                # 검증, 직렬화, 이미지, 일련번호
├── public/robots.txt         # 사이트는 색인 허용 (명함은 헤더로 따로 막는다)
├── firestore.rules           # Firestore 보안 규칙
├── firebase.json             # 호스팅 설정과 noindex 헤더
└── vite.config.ts
```
...
- [`src/utils/formValidation.ts`](src/utils/formValidation.ts) — `flattenFields` 가 항목 id 를 만드는 단일 기준입니다. 렌더링과 검증, 왼쪽 목차가 같은 규칙을 쓰도록 여기에 모아두었습니다.
...
- [`src/store/slices/authSlice.ts`](src/store/slices/authSlice.ts) — 익명 로그인과 계정 연결이 있는 곳. 로그인 상태(`user`)와 "계정 없이 이 기기에서만"(`isAnonymous`)을 구분해서 다룹니다.
...
Firebase 콘솔에서 **Authentication > 로그인 방법**의 `Google` 과 `익명` 을 모두 켜야 합니다. 익명이 꺼져 있으면 로그인하지 않은 사람은 명함을 만들 수 없습니다.
...
**Firestore 보안 규칙은 배포에 포함되지 않습니다.** `firestore.rules` 를 고쳤다면 따로 내보내야 합니다. 이 명령은 **지금 체크아웃된 브랜치의 파일**을 올리므로, 규칙이 들어 있는 브랜치에서 실행해야 합니다.
...
- **비회원 명함의 수명** — Firebase Authentication 의 익명 계정 30일 자동 정리를 켜 두었습니다. 계정이 사라져도 명함 문서는 그대로 남아 주인 없는 명함이 됩니다. 없는 명함 안내문은 "1개월이 지나면 사라진다"고 말하지만 실제 만료는 아직 구현되어 있지 않습니다. 명함에 만료 시각을 두고 지우는 방법과, 자동 정리를 끄는 방법 중 하나를 정해야 합니다.
- **비회원 명함 대량 생성 방지** — 익명 인증은 누구나 얼마든지 받을 수 있습니다. App Check 또는 서버 검증이 필요합니다.
- **회원 / 비회원 컬렉션 통합** — `cards` 와 `guestCards` 로 나뉘어 있어 조회할 때마다 양쪽을 봅니다. 이제 새 명함은 전부 `cards` 로 가므로, 남은 `guestCards` 를 옮기고 한쪽으로 합칠 수 있습니다. ([#32](https://github.com/hyeoniverse/QRU/issues/32))
