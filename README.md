...
- 로그인하면 **만든 명함을 모아 봅니다.** 이름, 일련번호, 만든 날짜, 셔플 노출 여부가 한눈에 들어옵니다.
- 저장했던 입력 원본을 그대로 불러와 **고칠 수 있습니다.** 공개로 두지 않았던 항목도 값이 남아 있습니다.
...
| `/mypage` | 내 명함 관리. 목록 · 수정 · 삭제 | ✅ 회원만 |

로그인은 Firebase Authentication 의 **Google 계정 로그인**을 씁니다. 비회원 명함은 아직 수정과 삭제가 막혀 있습니다. 비밀번호 확인을 클라이언트에서 할 수 없기 때문입니다.
...
- **비회원 명함 관리** — 회원은 `/mypage` 에서 고치고 지울 수 있지만 비회원은 아직 막혀 있습니다. 비밀번호 해시는 `private/card` 에 저장해두었으나, 검증은 서버에서만 할 수 있습니다. Cloud Functions 를 두거나, 비밀번호 대신 편집 토큰을 발급해 규칙의 `hashing.sha256` 으로 확인하는 방법 중 하나를 정해야 합니다.
- **일련번호로 명함 찾기** — 지금은 주소를 알아야만 명함을 엽니다. `allow list` 는 셔플 질의로만 열려 있고, 규칙은 질의에 담긴 값을 알 수 없어 `serialNumber` 로 거르는 것을 허용할 방법이 없습니다. 일련번호에서 문서 id 로 가는 포인터 컬렉션이 필요합니다.
